...
\block{Boolean masks}
\begin{lstlisting}
mask = (taxon == 'Primate')   # one boolean per stimulus
X[mask]                 # just those rows
mask.sum(); mask.mean() # how many; what FRACTION (a rate)
np.isin(cat, [0, 2])    # Part 3 codes: 0 Animal, 2 Face
\end{lstlisting}
\ret{Element-wise logic on arrays: \texttt{\&} and \ $\cdot$\ \texttt{|} or \ $\cdot$\ \texttt{\textasciitilde} not --- \emph{always with parentheses}: \texttt{(taxon == 'Primate') | (taxon == 'Rodent')}. Writing \texttt{and}/\texttt{or} between arrays raises ``truth value \dots\ is ambiguous''.}
...
\end{lstlisting}
\ret{\texttt{p0}: start near the right scale --- similarities run 0--10, so $a \approx 10$. \texttt{maxfev}: raise it, or hard fits die with ``Optimal parameters not found''. If a fit still fails, report which model and layer: a finding, not a bug.}
...
np.argmax(cum >= 0.85) + 1   # first True index; +1 = count
\end{lstlisting}
\ret{Settings go in the constructor, data goes to \texttt{.fit}, learned results grow a \emph{trailing underscore} (not a typo). \texttt{MDS} takes a dissimilarity matrix: \texttt{metric='precomputed'} (was \texttt{dissimilarity=} before 1.8). Random methods differ between runs unless you set \texttt{random\_state} --- reproducible is not the same as meaningful.}

\block{Per-group anything (groupby without groupby)}
\begin{lstlisting}
means = np.stack([fc6[obj == k].mean(axis=0)
...
\block{\texttt{silhouette\_samples} --- what goes in}
\begin{lstlisting}
sil = silhouette_samples(proj_fc6[:, :2], cat)  # 3g
\end{lstlisting}
\ret{First argument: the 2-D \emph{coordinates} in the PC1--PC2 plane, \emph{not} the RDM --- it computes its own distances. Second: the six category codes; \texttt{sil[cat == c].mean()} is that category's score (3g). Near 1 = sits apart \ $\cdot$\ near 0 = on a boundary \ $\cdot$\ negative = closer to another group than its own. No relation to 3f's silhouette \emph{masks}; the name is a coincidence.}
