...
\end{lstlisting}
\ret{\texttt{p0}: start near the right scale --- similarities run 0--10, so $a \approx 10$. \texttt{maxfev}: raise it, or hard fits die with ``Optimal parameters not found''. Wrap in \texttt{try/except RuntimeError} where a fit is \emph{allowed} to fail (2h says so --- that failure is a finding, not a bug).}
...
\begin{lstlisting}
silhouette_score(proj_fc6[:, :2], (cat == c).astype(int))
\end{lstlisting}
