...
The central move of this whole course: if you write down $D$ numbers about
something --- the pixel values of an image, the firing rates of $D$ neurons, a
person's rating on $D$ scales --- you can treat that list as a \emph{point} in a
$D$-dimensional space. The lectures write the vector for stimulus $i$ as
$\bm{x}^{(i)}$ and its $j$-th entry as $x^{(i)}_j$; $N$ stimuli make an
$N \times D$ matrix $\bm{X}$, one row per stimulus. Two things are ``similar'' when their points are close.
Everything else is detail about what \emph{close} should mean.

\subsection{Length, distance, and the dot product}

For two stimuli $\bm{a} = \bm{x}^{(i)}$ and $\bm{b} = \bm{x}^{(k)}$ in $\mathbb{R}^D$ (the lectures keep the superscripts; here $\bm{a}, \bm{b}$ are shorter):
\[
\|\bm{a}\| = \sqrt{\textstyle\sum_j a_j^2}, \qquad
d(\bm{a},\bm{b}) = \|\bm{a}-\bm{b}\|, \qquad
\bm{a}\cdot\bm{b} = \textstyle\sum_j a_j b_j = \|\bm{a}\|\,\|\bm{b}\|\cos\phi .
\]
...
Dividing out the lengths leaves only the angle $\phi$ between the two vectors:
\[
\cos\phi = \frac{\bm{a}\cdot\bm{b}}{\|\bm{a}\|\,\|\bm{b}\|}.
\]
...
The lectures name the three dissimilarities this way, and Assignment~1 \S1b
asks you to write all three as \texttt{euclidean(X)}, \texttt{cosine\_dist(X)}
and \texttt{correlation\_dist(X)}, each returning the $N \times N$ matrix over
the rows of \texttt{X}:
\[
d_{\mathrm{euc}} = \|\bm{a}-\bm{b}\|, \qquad
d_{\cos} = 1 - \cos\phi, \qquad
d_{\mathrm{corr}} = 1 - r,
\]
where $r$ is the Pearson correlation of $\bm{a}$ and $\bm{b}$: the cosine after
each vector's own mean is subtracted. The last two lie in $[0, 2]$.
...
on the diagonal; entry $d_{ik}$ is the dissimilarity of stimuli $i$ and $k$ (the
lectures call it the RDM, representational dissimilarity matrix). That is $N^2$
numbers to store $N(N-1)/2$ facts, so SciPy also
...
two answers unless you fix it. The call the lectures use:

\begin{lstlisting}
mds = MDS(n_components=2, metric='precomputed', n_init=8,
          normalized_stress='auto', random_state=0)
Y = mds.fit_transform(D)            # (N, 2); mds.stress_ afterwards
\end{lstlisting}

Add \texttt{metric\_mds=False} for non-metric MDS, which fits only the rank
order of the dissimilarities. \texttt{metric='precomputed'} is the scikit-learn
$\geq 1.8$ spelling (\texttt{dissimilarity='precomputed'} before); the
Assignment~1 setup cell installs 1.8 for you.
...
\item \textbf{Mean, variance, standard deviation.} The center, the spread, and
      ...
      a special hazard: correlation divides by a standard deviation, so if you
      $z$-score each \emph{unit} across stimuli (Assignment~1 \S3h) a unit that
      never responds divides by zero and puts a silent \texttt{nan} into
      everything downstream. Correlation \emph{between two images} across their
      units (\S1b) does not fail on a dead unit, but it is biased by it: see
      exercise~10.
...
\item \textbf{Pearson vs.\ Spearman correlation.} Pearson ($r$) measures how
      well a \emph{straight line} fits; Spearman ($\rho$) does the same after
      replacing values by their ranks, so it measures whether two things increase together
...
\item \textbf{Degenerate rows or columns.} Anything with zero variance breaks
      a correlation taken along that axis, because you divide by its standard
      deviation.
...
\item \texttt{(X == 0).mean()}. Network activations are full of exact zeros
      because of the \texttt{ReLU} nonlinearity. A unit that is \emph{always}
      zero has no variance, so $z$-scoring it across stimuli divides by zero;
      and zeros that two images \emph{share} pull both means down after
      centering and then count as agreement, which inflates their correlation
      (Assignment~1 \S2c). Decide the unit set once, for every image, and say
      what you did.
\end{enumerate}
